...
void dnr(Mat &img, Mat& out2, vector<Mat>& detectList )
{
  ...
  // imshow("yo",out2);
   // imshow("out",out);
    ...
    {  Rect r = boundingRect(mser_points[i]);
        ...
        float aspect = (float)r.height/r.width;
        int mserarea = mser_points[i].size();
        double area_ratio = (double)mserarea/(r.width*r.height);


        if((roi.rows>25 && roi.rows<80) && (roi.cols>25 && roi.cols<80) && (aspect>0.5 && aspect<1.5) && (area_ratio>0.4 && area_ratio <1))
        {   
...
{	   //cap.set(CV_CAP_PROP_FRAME_WIDTH, 640);
    ...
    Mat RBimg = normalizeBR(img);
    imshow("yoyoyo",RBimg);
    Mat GREYimg;
    cvtColor(img,GREYimg,CV_BGR2GRAY);
    dnr(img, RBimg, detectList);
    dnr(img, GREYimg, detectList);
    //Mat out=img.clone();
